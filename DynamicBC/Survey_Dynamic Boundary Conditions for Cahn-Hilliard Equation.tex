...
\begin{tikzpicture}[every node/.style={rectangle,fill=cyan!30,rounded corners}]
    ...
    \node[above right=of GMS] (CF) {Colli, Fukao (2015) \cite{ColliFukao2015-GMS}};
...
\node[right=of S] (CF) {Colli Fukao (2015) \cite{ColliFukao2015-CH-AC}};
...
{\bf 先行結果}：
\begin{itemize}
	\item Colli, Fukao (2014) \cite{ColliFukao2014} 内部でAllen--Cahn, 境界でAllen--Cahn方程式をみたす動的境界問題, 本論文と同様の手法を用いているので参考になる.
\end{itemize}
...
\subsection{Colli Fukao (2015) \cite{ColliFukao2015-CH-AC} note:null day:2019.10.08}
{\bf 論文情報}：
...
\subsection{Colli, Hassan Farshbaf-Shaker, Gilardi, Sprekels (2015) \cite{ColliFGS2015} note:null day:2019.10.10}
{\bf 論文情報}：
...
{\bf 主問題}：
内部ではviscous Cahn--Hilliard方程式又はpure Cahn--Hilliard方程式をみたす次の問題を考える.
...
{\bf Theorem 2.2}.
条件(A1)-(A4)をみたすとき, 初期値$y_{0,1},y_{0,2}\in V$に対する開は初期値と外力項に関して連続依存性を持つ.

{\bf Theorem 2.3}.
条件(A1)-(A6)をみたすとき, $y_0\in \mathcal{V}, \hat{\beta}(y_0)\in \Lone$ and $\hat{\beta}\sG(y_0|\sG)\in L^1(\Gamma)$なる初期値に対して弱開が存在する.
...
\begin{itemize}
	...
	\item[(A3)] (時空間に陽に依存する部分) ((2.5),(2.7)に相当) $\lambda\in L^{\infty}(Q)$ and $\lambda\sG\in L^{\infty}(\Sigma)$, 特に$\tau=0$のとき, $\lambda\in L^{\infty}(0,T;W^{1,3}(\Omega))$.
